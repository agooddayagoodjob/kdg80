...
- Typography-layer не может ехать в длительном scale/drift вместе со всей сценой: после entrance крупный текст остаётся геометрически стабильным.
- Для weekly-title запрещён `clip-path` / `mask` reveal, который выглядит как дешёвая шторка.
- Предпочтительный text entrance: words-only reveal, где слова мягко приходят снизу с коротким opacity-rise.
...
- Default placement для ФИО и регалий спикера: в copy-column под названием лекции. Overlay-плашка на фигуре не используется по умолчанию и требует отдельного подтверждения.
...
- Первый кадр интро уже читается как готовый кадр, а не как пустая сцена перед проявлением.
...
- Крупный текст не даёт лесенки из-за scene-wide scaling.
